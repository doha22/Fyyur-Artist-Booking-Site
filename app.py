...
def venues():
  ...
  for venue in venues:
        res = Venue.query.filter_by(state=venue.state).filter_by(city=venue.city).all()
        venue_data = []
...
def search_venues():
  ...
  for res in search_result:
      ...
      "id": res.id,
      "name": res.name,
      "num_upcoming_shows": len(db.session.query(Show).filter(Show.venue_id == res.id).filter(Show.start_time > datetime.now()).all()),
    })
...
def show_venue(venue_id):
  ...
  # as (select *from Venue where Venue.id = venue_id) 
  venue = db.session.query(Venue).filter(Venue.id == venue_id).one()

  # select * from Show where Show.venue_id = venue_id
  list_shows = db.session.query(Show).filter(Show.venue_id == venue_id)
  past_shows = []
  upcoming_shows = []

  # make loop through the previous show query to get each show_data for that condition
  # artist.id = show.artist_id
  
  for show in list_shows:
      # select artist.name and artist.image_link from artist , show where artist.id = show.artist_id
...
def create_venue_submission():
  ...
  try:
    ...
    # created_venue = Venue(name=name, genres=genres, address=address,city=city, state=state, phone=phone, facebook_link=facebook_link, image_link=image_link)
...
def search_artists():
  ...
  # get in Artist db and passing which term will search by 
  # using ilike 
  search_term = request.form.get('search_term', '')
...
def show_artist(artist_id):
  ...
  # select * from Artist where Artist.id = artist_id
  artist = db.session.query(Artist).filter(Artist.id == artist_id).one()

  # select * from Show where Show.artist_id = artist_id
  ...
  for show in list_shows:
      # select venue.name and venue.image_link from venue , show where venue.id = show.venue_id
